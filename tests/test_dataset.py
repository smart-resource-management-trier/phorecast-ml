...
class TestDataset(unittest.TestCase):
    ...
    def test_windowing(self, window_size, stride, max_missing):
        ...
        windows = phorecast_ml.preprocessing.windowing(test_dataset, window_size=window_size, stride=stride,
    ...
    def test_split_windows(self, test_ratio, factor):
        weeks_in_test = 1
        windows = phorecast_ml.preprocessing.windowing(dataset.copy(), window_size=24, stride=6, max_missing=3)
        train, test = phorecast_ml.preprocessing.split_windows(windows, test_ratio=test_ratio, weeks_in_test=weeks_in_test,
    ...
    def test_get_dataset_from_windows(self, window_size, stride, max_missing):
        windows = phorecast_ml.preprocessing.windowing(dataset.copy(), window_size=window_size, stride=stride,
                                                       max_missing=max_missing)
        cols_length = len(windows[0].columns)
        train, test = phorecast_ml.preprocessing.split_windows(windows)
        (train_x, train_y), index = phorecast_ml.preprocessing.get_dataset_from_windows(train, target="")
        ...
        (train_x, train_y), index = phorecast_ml.preprocessing.get_dataset_from_windows(train, target="WR01")
...
class TestAttachSolarPositions(unittest.TestCase):
    ...
    def test_attach_solar_positions(self):
        ...
        modified_df = phorecast_ml.preprocessing.attach_solar_positions(self.df.copy(), self.latitude, self.longitude)
    ...
    def test_error_on_existing_solar_positions(self):
        ...
        with self.assertRaises(ValueError):
            phorecast_ml.preprocessing.attach_solar_positions(df_with_solar, self.latitude, self.longitude)
...
class TestCreateTFDatasetMultipleInputs(unittest.TestCase):
    ...
    def test_single_array_input(self):
        ...
        dataset = phorecast_ml.preprocessing.create_tf_dataset(self.features, self.batch_size)
    ...
    def test_tuple_with_empty_labels(self):
        ...
        dataset = phorecast_ml.preprocessing.create_tf_dataset((self.features, self.empty_labels), self.batch_size)
        for batch in dataset:
            self.assertEqual(batch.numpy().shape[1], 10)  # Check feature size

    def test_tuple_with_labels(self):
        # Test input tuple with non-empty labels
        dataset = phorecast_ml.preprocessing.create_tf_dataset((self.features, self.labels), self.batch_size)
